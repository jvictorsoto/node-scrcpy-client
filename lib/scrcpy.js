...
class Scrcpy extends EventEmitter {
  ...
  async start() {
    ...
    const device = this._config.deviceId ? devices.find(d => d.id === this._config.deviceId) : devices[0];
    if (!device) {
    ...
    // Transfer server...
    await this.adbClient.push(device.id, path.join(__dirname, 'scrcpy-server.jar'), '/data/local/tmp/scrcpy-server.jar')
